...
let TasksService = class TasksService {
    constructor(tasksRepository, projectsService, usersService) {
    ...
    async create(createTaskDto) {
        ...
        if (existingTask) {
            throw new common_1.HttpException('Task already exists', common_1.HttpStatus.BAD_REQUEST);
        }
        const existProject = await this.projectsService.findOne(createTaskDto.project_id);
        if (!existProject) {
            throw new common_1.HttpException('Project does not exist', common_1.HttpStatus.BAD_REQUEST);
        }
